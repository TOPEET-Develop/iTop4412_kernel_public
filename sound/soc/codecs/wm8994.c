...
#include <sound/tlv.h>
#include <sound/soc-dapm.h>			// zsb add
 ...
 */
static const char *playback_path_name[] = { "None","Speaker Nomal","Headphone Nomal","Receiver Nomal","Spk_Hp Nomal",
					...
					"Headphone incall","Spk_Hp incall","BT incall"};

static const char *capture_path_name[] = { "None","Main_Mic Nomal","Sencond_Mic Nomal","Hand_Mic Nomal","Main_Mic incall",
					"Sencond_Mic incall","Hand_Mic incall",};
/* End here */
...
static int tc4_get_playback_path(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{	
	struct snd_soc_codec *codec = snd_kcontrol_chip (kcontrol);
	struct wm8994_priv *wm8994 = snd_soc_codec_get_drvdata(codec);

  	ucontrol->value.integer.value[0] = wm8994->playback_path;
	return 0;
}

static int tc4_set_playback_path(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip (kcontrol);
	struct wm8994_priv *wm8994 = snd_soc_codec_get_drvdata(codec);
...
//	if(wm8994->playback_path != user_value)		// zsb, force to update path setting
	{
		ret = set_playback_path(codec, user_value, wm8994->capture_path, codec->bias_level);	
		if(ret >= 0)	
			wm8994->playback_path= user_value;
	}
	return ret;
}

static int tc4_get_capture_path(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{	
	struct snd_soc_codec *codec = snd_kcontrol_chip (kcontrol);
	struct wm8994_priv *wm8994 = snd_soc_codec_get_drvdata(codec);

  	ucontrol->value.integer.value[0] = wm8994->capture_path;
	return 0;
}

static int tc4_set_capture_path(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip (kcontrol);
	struct wm8994_priv *wm8994 = snd_soc_codec_get_drvdata(codec);
...
//	printk(KERN_INFO "%s(): path => %s\n", __func__,capture_path_name[user_value]);	
	//if(wm8994->capture_path != user_value) //wenpin.cui: always do this
	{
		ret = set_capture_path(codec, wm8994->playback_path, user_value, codec->bias_level);
		if(ret >= 0)
			wm8994->capture_path= user_value;
	}
	return ret;
}
...
static void wm8994_update_class_w(struct snd_soc_codec *codec)
{
	...
	dprintk("[zsb] wm8994_update_class_w\n");
...
static const struct snd_soc_dapm_route intercon[] = {
	...
	{ "AIF3ADC Mux", "AIF1ADCDAT", "AIF1ADCDAT"}, //wenpin.cui
	{ "AIF3ADC Mux", "AIF2ADCDAT", "AIF2ADCDAT"}, //wenpin.cui
	{ "AIF3ADC Mux", "AIF2DACDAT", "AIF2DACDAT"}, //wenpin.cui
	...
	{ "AIF3ADCDAT", NULL, "AIF3ADC Mux" }, //wenpin.cui

	/* AIF3 output */
//	{ "AIF3ADCDAT", "AIF1ADCDAT", "AIF1ADC1L" },//wenpin.cui
//	{ "AIF3ADCDAT", "AIF1ADCDAT", "AIF1ADC1R" },//wenpin.cui
//	{ "AIF3ADCDAT", "AIF1ADCDAT", "AIF1ADC2L" },//wenpin.cui
//	{ "AIF3ADCDAT", "AIF1ADCDAT", "AIF1ADC2R" },//wenpin.cui
//	{ "AIF3ADCDAT", "AIF2ADCDAT", "AIF2ADCL" }, //wenpin.cui
//	{ "AIF3ADCDAT", "AIF2ADCDAT", "AIF2ADCR" }, //wenpin.cui
//	{ "AIF3ADCDAT", "AIF2DACDAT", "AIF2DACL" }, //wenpin.cui
//	{ "AIF3ADCDAT", "AIF2DACDAT", "AIF2DACR" }, //wenpin.cui
...
#ifdef TC4 //wenpin.cui: only one DAI
#else
...
#ifdef TC4 /*wenpin.cui: we only have one dai*/
#else
	{
		.name = "wm8994-aif2",
		.id = 2,
...
/*
 * wenpin.cui:cache sync is new API since kernel 3.0
 * it seems audio won't work correctly after resume
 * currently mark it temp
 */
#if 0
 ...
 *	0:Output
 */
static int wm8994_voicecall_common_setting(struct snd_soc_codec *codec)
{
	/*GPIO Configuration*/
	wm8994_write(codec, WM8994_GPIO_1,  0x0001);//gpio1 must be set, or mic won't work
	wm8994_write(codec, WM8994_GPIO_2,  0x8100);//mclk2
	/*AIF2 GPIO*/
	wm8994_write(codec, WM8994_GPIO_3,  0x8100);//bclk2	
	wm8994_write(codec, WM8994_GPIO_4,  0x8100);//lrclk2
...
#ifdef TC4
	/* Reset all registers of wm8994 */
	snd_soc_write(codec, WM8994_SOFTWARE_RESET, 0x0000);
	/* Initialize wm8994 GPIO configure */
	wm8994_voicecall_common_setting(codec);		
#endif
...
/* We don't use wm8994 irq */
#ifdef TC4
	; /* do nothing, mic and hp detect codes located at switch_gpio.c */
#else
	switch (control->type) {
	case WM8994:
