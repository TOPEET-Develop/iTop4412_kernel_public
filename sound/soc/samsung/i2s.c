	  ...
	  int clk_id, unsigned int rfs, int dir)
{
	...
	case SAMSUNG_I2S_RCLKSRC_1: /* clock corrsponding to IISMOD[10] := 1 */
		...
		if (!any_active(i2s)) {
			...
			/* wenpin.cui:Dec 13 2011
			 * if we get here, the primary i2s is busy
			 * however, it indicates i2s's clock has been set correctly
			 * if we set op_clk from other i2s DAI, which doesn't exist on TC4
			 * we'll get kernel panic
			 */
#if 0
	...
	int div_id, int div)
{
	...
	case SAMSUNG_I2S_DIV_PRESCALER:
		writel(((div - 1) << 8) | PSR_PSREN, i2s->addr + I2SPSR);	// zsb add
		break;
	default:
		dev_err(&i2s->pdev->dev,
