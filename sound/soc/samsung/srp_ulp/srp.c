...
#define SRP_DEV_MINOR		(253)	//Robin, the minor 250 is already use by the pmic in tc4 evt board.so change 250 -> 253
...
/*
 * wenpin.cui:
 * Create a interface for Awesomeplayer to detemine whether using ULP or not
 */

#define A2DP_ULP
#ifdef A2DP_ULP
static DEFINE_SEMAPHORE(a2dp_state_mutex);
static char bt_state[32] = "not initialized";
#endif
...
/*
 * wenpin.cui:
 * AudioPolicyManager store the a2dp device state,
 * When playing audio, Awesomeplayer need this info.
 */
#ifdef A2DP_ULP
static ssize_t a2dp_state_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{

	down(&a2dp_state_mutex);
	if (strncpy(buf, bt_state, strlen(bt_state)) == NULL) {
		up(&a2dp_state_mutex);
		printk(KERN_ERR "%s: strncpy failed\n", __func__);
		return -1;
	}
	up(&a2dp_state_mutex);

	printk(KERN_DEBUG "[wenpin] %s: %s\n", __func__, buf);

	return strlen(bt_state)+1;
}

static ssize_t a2dp_state_store(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t count)
{
	if (count > 32) {
		printk(KERN_ERR "[wenpin] %s invalid length %d\n", __func__, count);
		return -1;
	}

	down(&a2dp_state_mutex);
	memset(bt_state, 0, 32); /* remove unpleasant characters */
	if (strncpy(bt_state, buf, count) == NULL) {
		up(&a2dp_state_mutex);
		printk(KERN_ERR "%s: strncpy failed\n", __func__);
		return -1;
	}
...
static DEVICE_ATTR(a2dp_state, 0644, a2dp_state_show, a2dp_state_store);
#endif
...
#ifdef A2DP_ULP
	ret = device_create_file(&pdev->dev, &dev_attr_a2dp_state);
	if (ret)
		printk(KERN_ERR "WENPIN: a2dp sysfs create failed\n");
#endif
